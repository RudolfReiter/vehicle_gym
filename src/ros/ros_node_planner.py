...
from vehiclegym.trajectory_planner_acados_iterative import VehiclePlannerAcados2022Iter
from vehiclegym.trajectory_planner_acados_20221 import PlannerOptions
...
class MPCRL(Node):
    def __init__(self):
        ...
        self.road_options = RoadOptions()
        self.current_road = None
        road_model_planner = Road(road_options=self.road_options)
        road_model_planner.s_grid_ = np.linspace(-50, 1000, num=1050)
        road_model_planner.kappa_grid_ = np.zeros_like(road_model_planner.s_grid_)
        road_model_planner.nl_grid_ = np.ones_like(road_model_planner.s_grid_) * 10
        road_model_planner.nr_grid_ = np.ones_like(road_model_planner.s_grid_) * 10

        # Create planner
        self.planner = VehiclePlannerAcados2022Iter(
            ego_model_params=self.ego_model_params,
            road=road_model_planner,
    ...
    def initialize(self):
        ...
        self.planner.initialize_iter(
            states_ego=initial_state,
    ...
    def main_iteration(self):
        ...
        self.current_road.update_from_xy(p_xy=xy_grid, nl=nl_grid, nr=nr_grid, s=s_grid)

        state = self.get_ego_state_f()

        # set states and actions, main solver call
        self.planner.solve_iter(
            states_ego=state,
            states_opp=[],
            actions=self.default_action,
            s_grid=s_grid,
