...
from vehicle_models.model_kinematic_cartesian import KinematicModelCartesianSmall, KinematicModelCartesian
...
class NmpcOptions20224:
    ...
    weight_inputs: np.ndarray = np.array([1e-5, 1e6])
    weight_states: np.ndarray = np.array([1e1, 1e1, 1e-4, 0, 0])
    weight_states_end: np.ndarray = np.array([1e0, 1e0, 1e-1, 0, 0])
...
class Nmpc20224:
    def __init__(self,
                 model_vehicle: KinematicModelCartesian,
                 planner_options: NmpcOptions20224,
                 x0: np.ndarray = np.array([0, 0, 0, 0, 0])):
        # create render arguments
        self.casadi_model = model_vehicle
        self.planner_opts = planner_options
        ocp = AcadosOcp()
        self.surrogate_delta = 0
        ...
        ocp.cost.zl = np.array([1e4, 1e3, 1e3])
        ocp.cost.Zl = np.array([1e4, 0, 0])
    ...
    def get_acceleration(self):
        ...
        return self.casadi_model.f_a_lat_cg(x, u).full()[0]

    def step(self, x_measured: np.ndarray, y_ref: np.ndarray) -> [np.ndarray, int]:

        x_measured[-1] = self.surrogate_delta

        self.solver.set(0, "lbx", x_measured)
        self.solver.set(0, "ubx", x_measured)
        terminal_constraint_l = np.array([-self.casadi_model.params_.maximum_lateral_acc,
                                          y_ref[0, self.planner_opts.n_nodes],
                                          y_ref[1, self.planner_opts.n_nodes]])
        terminal_constraint_u = np.array([self.casadi_model.params_.maximum_lateral_acc,
                                          y_ref[0, self.planner_opts.n_nodes],
                                          y_ref[1, self.planner_opts.n_nodes]])
    ...
    def get_surrogate_controls(self) -> Tuple[float, float]:
        u0 = self.solver.get(0, "u")
        force = u0[0]
        x_plus = self.solver.get(1, "x")
        delta = (self.surrogate_delta + x_plus[4]) / 2
        self.surrogate_delta = x_plus[4]
        return force, delta
...
if __name__ == "__main__":
    ...
    model = KinematicModelCartesian(params)
    ...
    for i in range(Nsim):
        ...
        for stage in range(controller.planner_opts.n_nodes + 1):
            y_ref[:, stage] = np.array([pos_x_ref[i + stage], pos_y_ref[i + stage], theta_ref[i + stage], 10, 0, 0, 0])
